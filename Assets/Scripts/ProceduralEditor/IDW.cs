...
public static class IDW
{
    ...
    static public float[] inverseDistanceWeightings(Vector4[] point_cloud, float[] x, float[] z, float old_base = 0.0f)
    {
        float[] ys = new float[x.Length];
        for (int i = 0; i < ys.Length; i++)
        {
            ys[i] = inverseDistanceWeighting(point_cloud, x[i], z[i]);
        }
        return ys;
    }
}