...
static public class DouglasPeuckerAlgorithm
{
    ...
    static public List<Vector2> DouglasPeucker2D(List<Vector2> points, float epsilon)
    {
        int select_index = 0;
        float d, dmax = 0;
        Vector2 point_tail = points[points.Count - 1];
        for (int point_index = 2; point_index < points.Count; point_index++)
        {
            d = perpendicularDistance2D(points[point_index], points[0], point_tail);
            if (d > dmax)
            {
                dmax = d;
                select_index = point_index;
        ...
        if (dmax > epsilon)
        {
            List<Vector2> results1 = DouglasPeucker2D(points.GetRange(0, select_index + 1), epsilon);
            List<Vector2> results2 = DouglasPeucker2D(points.GetRange(select_index, points.Count - select_index), epsilon);
            results2.RemoveAt(0);
            results1.AddRange(results2);
            return results1;
        }
    ...
    static public float perpendicularDistance2D(Vector2 p, Vector2 p1, Vector2 p2)
    {
        float A = p.x - p1.x;
        float B = p.y - p1.y;
        float C = p2.x - p1.x;
