...
public class RoadIntegration : MonoBehaviour
{
    ...
    int roadCanLinked(string road_id)
    {
        if (bicycle_way_list.Count == 0)
            return GetComponent<OSMReaderManager>().osm_reader.getPathIndex(road_id);

        int pathes_index = GetComponent<OSMReaderManager>().osm_reader.getPathIndex(bicycle_way_list[bicycle_way_list.Count - 1]);
        ...
        if (edit_mode)
        {
            ...
            if (new_road_index != -1)
            {
                ...
                if (bicycle_way_list.Count == 1)
                {
                    ...
                    bicycle_points_list = new List<string>(GetComponent<OSMReaderManager>().osm_reader.pathes[new_road_index].ref_node);
                }
                else
                {
                    ...
                    {
                        if (GetComponent<OSMReaderManager>().osm_reader.points_lib[bicycle_points_list[bucycle_points_index]].connect_way.Contains(road_id))
                        {
                            break;
                        }
                        else
                    ...
                    if (GetComponent<OSMReaderManager>().osm_reader.pathes[new_road_index].head_node == bicycle_points_list[bucycle_points_index]) // head to tail
                    {
                        for (int new_road_ref_index = 0; new_road_ref_index < GetComponent<OSMReaderManager>().osm_reader.pathes[new_road_index].ref_node.Count; new_road_ref_index++)
                        {
                            bicycle_points_list.Add(GetComponent<OSMReaderManager>().osm_reader.pathes[new_road_index].ref_node[new_road_ref_index]);
                        }
                    }
                    else
                    {
                        for (int new_road_ref_index = GetComponent<OSMReaderManager>().osm_reader.pathes[new_road_index].ref_node.Count - 1; new_road_ref_index >= 0; new_road_ref_index--)
                        {
                            bicycle_points_list.Add(GetComponent<OSMReaderManager>().osm_reader.pathes[new_road_index].ref_node[new_road_ref_index]);
                        }
                    }
    ...
    void writeBPF(string file_path)
    {
        ...
        using (StreamWriter sw = new StreamWriter(file_path))
        {
            ...
            foreach (KeyValuePair<string, Node> point in GetComponent<OSMReaderManager>().osm_reader.points_lib)
            {
